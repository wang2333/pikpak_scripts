...
class PikPak {
      ...
      try {
        ...
        if (e.response.data.error && retries <= 3) {
          ...
          await this.captchaInit(
            `${config.method.toUpperCase()}:${new URL(config.url).pathname}`
          );
  ...
  disposeConfig(config) {
    ...
    if (
      newConfig.headers &&
      newConfig.headers["x-captcha-token"] !== undefined
    ) {
  ...
  async captchaInit(action) {
    ...
    const captchaSign = getCaptchaSign(
      this.package_name,
      this.app_client_id,
      this.client_version,
      this.deviceid,
  ...
  async getVerificationCode(mailjs) {
    console.log("开始获取验证码...");
    for (let i = 0; i < 10; i++) {
      const { data } = await mailjs.getMessages();
      console.log(`获取到邮件数量: ${data.length}`);
      if (data.length > 0) {
        let intro = data[0].intro;
        const regex = /\d+/g;
        const matches = intro.match(regex);

        if (matches && matches[0]) {
          console.log("提取到的验证码是:", matches[0]);
          this.verification_code = matches[0];
          return;
        } else {
